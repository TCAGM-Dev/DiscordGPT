(async ()=>{
  ...
  async function sendGPTMessage(user, ...args) {
    if (!Object.keys(apis).includes(user.id)) {
      const key = await db.get(`apikey.${user.id}`)
      if (key) {
        apis[user.id] = new ChatGPTAPI({apiKey: key})
      } else {
        apis[user.id] = apis["default"]
      }
    }
    const api = apis[user.id]
    return api.sendMessage(...args).catch(err => err.message)
  }
